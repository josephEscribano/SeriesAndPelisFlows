...
class FragmentMostrarSeries : Fragment() {
    ...
    private fun configContextBar() = object : ActionMode.Callback {
        ...
        override fun onActionItemClicked(p0: ActionMode?, item: MenuItem?): Boolean {
            return when(item?.itemId){
                R.id.visto -> {
                    Snackbar.make(binding.root,"Visto",Snackbar.LENGTH_LONG)
                    return true
                }
                else -> false
            }
            ...
            with(binding){
                ...
                binding.seasonSpinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener,
                    ...
                    override fun onItemSelected(p0: AdapterView<*>?, p1: View?, p2: Int, p3: Long) {
                        it?.temporadas?.get(p2)?.let { temporada ->  viewModel.getCapitulo(it.idApi, temporada.seasonNumber) }
                    }
    ...
    private fun addCaps(serie: Serie?)  {
        serie?.temporadas?.forEach { temporada ->  viewModel.getCapitulo(serie.idApi, temporada.seasonNumber)}
        viewModel.capituloData.observe(this, {
    ...
    override fun onOptionsItemSelected(menuItem: MenuItem): Boolean {
        when (menuItem.itemId) {
            R.id.favoritos -> {
                ...
                Toast.makeText(this.requireContext(), "Serie añadida" ,Toast.LENGTH_SHORT).show()
                true
            }
            else -> false
        }
