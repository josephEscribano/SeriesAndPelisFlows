...
class RemoteDataSource @Inject constructor(
    ...
    suspend fun getAll(titulo: String, region: String) =
        safeApiCall(apicall = { multiMediaService.getAll(titulo, region) }, transform = {
            it.resultMultimediaPojos.map { it.toMultimedia() }
        })


    suspend fun getPopularMovies() =
        safeApiCall(apicall = { multiMediaService.getPopularMovie() }, transform = {
            it.resultPopularMoviePojos.map { resultPopularMoviePojo -> resultPopularMoviePojo.toMultimedia() }
        })

    suspend fun getPopularSeries() =
        safeApiCall({ multiMediaService.getPopularSeries() }, transform = {
    ...
    suspend fun getSerie(tvId: Int) =
        safeApiCall(apicall = { serieService.getSerie(tvId) }, transform = SeriePojo::toSerie)

    suspend fun getActor(actorId: Int) =
        safeApiCall(apicall = { actorService.getActor(actorId) }, transform = ActorPojo::toActor)

    suspend fun getCapitulos(tvId: Int, seasonNumber: Int) =
        safeApiCall(apicall = { serieService.getCapitulos(tvId, seasonNumber) }, transform = {
